...
#{
#if [ ! -e "$RESOLVEDIR" ]; then
#echo "#######  Instalando Resolve 16.2  #########" ##### 
#cp -f $SOURCEDIR/DaVinci_Resolve_Studio_16.2_Linux.run $INSTALLDIR/DaVinci_Resolve_Studio_16.2_Linux.run
#cd $INSTALLDIR
#chmod 777 DaVinci_Resolve_Studio_16.2_Linux.run
#sudo -u $USERNAME  -H sh -c "./DaVinci_Resolve_Studio_16.2_Linux.run -i -y"
#fi
#}
...
if [ ! -e "$RESOLVEDIR" ]; then
echo "#######  Instalando Resolve 16.2  #########" ##### 
cp -f $SOURCEDIR/DaVinci_Resolve_16.2_Linux.run $INSTALLDIR/DaVinci_Resolve_16.2_Linux.run
cd $INSTALLDIR
chmod 777 DaVinci_Resolve_16.2_Linux.run
sudo -u $USERNAME  -H sh -c "./DaVinci_Resolve_16.2_Linux.run -i -y"
fi
}
############   Fusion 9  ###############
#if [ ! -d "$FUSIONDIR" ]; then
...
if [ ! -e "$PFTRACKFILE" ]; then
 ...
 echo "#######  Instalando PFTRACK 2018  #########" ##### 
cp -f $SOURCEDIR/pftrack-2018.15.14-1.x86_64.rpm $INSTALLDIR/pftrack-2018.15.14-1.x86_64.rpm
cd $INSTALLDIR
chmod 777 pftrack-2018.15.14-1.x86_64.rpm
rpm -Uvh pftrack-2018.15.14-1.x86_64.rpm
fi
